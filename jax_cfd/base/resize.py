...
def top_hat_downsample(
  ...
  assert sum([round(f) == f for f in factor])
  assert sum([round(f) == f for f in filter_size])  # this can be relaxed
  assert sum(abs(
      jnp.array(filter_size) % 2)) == 0  # only even filters are implemented
  assert sum(abs(
      jnp.array(factor) % 2)) == 0  # only even factors are implemented
  result = []
  for c in variables:
    ...
    for axis in range(c.grid.ndim):
      ...
      axes = list(np.arange(source_grid.ndim))
      axes.remove(axis)
      for ax in axes:
        convolve_1d = jax.vmap(convolve_1d, in_axes=int(ax), out_axes=int(ax))
    ...
    c = grids.GridVariable(c_centered, boundary)
    result.append(c)
  return tuple(result)
